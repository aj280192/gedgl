...
class DeepwalkDataset:
    def __init__(self, args):
        ...
        Parameter : use args from getting input parameters
        ---------
            ...
            negative int : negative samples for each positive node pair
        ...
        # random walks
        start = time.time()
        walks = torch.tensor(self.random_walks_generator())
    ...
    def random_walk(self, start_node):
        ...
        for i in range(walk_length - 1):
            node = walk[i]
            neighbors = sorted([int(x) for x in G.successors(node)])
    ...
    def random_walks_generator(self):
        ...
        for node in nodes:
            node_walks = []
            for _ in range(G.num_walks):
                node_walks.append(self.random_walk(node))
            walks.append(node_walks)
            # print("node = ", node, "node_walks = ", node_walks)
