...
class DeepwalkTrainer:
    def __init__(self, args):
        ...
        """ Setting parameter relating to Deepwalk Training"""
    ...
    # define another __init__ to call in other module
    """
    def __init__(self,
                 net_file,
                 # map_file,
    ...
    def train(trainer, args):
        ...
        if args.num_procs > 1:
            trainer.fast_train_mp()
        else:
            trainer.fast_train()
    """

    def fast_train_mp(self, args):
        ...
        for i in range(np_):
            p = mp.Process(target=self.fast_train_sp, args=(i, args))
            ps.append(p)
            p.start()
    ...
    @thread_wrapped_func
    def fast_train_sp(self, gpu_id, args):
        """ a subprocess for fast_train_mp """
        if args.mix:
            self.emb_model.set_device(gpu_id)
        torch.set_num_threads(args.num_threads)

        num_batches = len(self.dataset.net) * args.num_walks / args.batch_size
        num_batches = int(num_batches)
        print("num batches: %d in subprocess [%d]" % (num_batches, gpu_id))
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                idx = int(i % num_batches)
                walks = self.dataset.walks[idx * args.batch_size:(idx + 1) * args.batch_size]
    ...
    def fast_train(self, args):
        """ fast train in single process """
        # the number of positive node pairs of a node sequence
        num_pos = 2 * args.walk_length * args.window_size - args.window_size * (args.window_size + 1)
        num_pos = int(num_pos)

        trainer.init_device_emb()

        num_batches = len(self.dataset.net) * args.num_walks / args.batch_size
        num_batches = int(num_batches)
        # print("num_batches = ", num_batches)
        # num_batches = len(np.ceil(num_batches))
        ...
        with torch.no_grad():
            ...
            for iteration in range(self.iterations):
                ...
                random.shuffle(self.dataset.walks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net_file', type=str, default="sample_graph1.tsv",
    ...
    """ train the embedding """
    if args.num_procs > 1:
        trainer.fast_train_mp(args)
    else:
        trainer.fast_train(args)

    end_time = time.time()
    print("Total used time: %.2f" % (end_time - start_time))
